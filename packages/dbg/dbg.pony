...
use @dbg_ctx_impl_file_create[DbgCtx](number_of_bits: USize,
      dst: Pointer[U8])
use @dbg_ctx_impl_mem_create[DbgCtx](number_of_bits: USize,
...
class DC
  ...
  new create_with_dst_stdout(number_of_bits: USize) =>
    _dst_stream = None
    _dc = @dbg_ctx_impl_file_create(number_of_bits,
  ...
  =>
    _dst_stream = None
    _dc = @dbg_ctx_impl_mem_create(number_of_bits, dst_size, tmp_buf_size)
